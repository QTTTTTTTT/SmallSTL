...
namespace SmallSTL{
	template<class T>
		...
		void dq_iter<T>::swap(dq_iter& it){
			SmallSTL::swap(mapIndex_, it.mapIndex_);
			SmallSTL::swap(cur_, it.cur_);
		}
		template<class T>
	...
	void deque<T, Alloc>::push_back(const value_type& val){
		...
		SmallSTL::construct(end_.cur_, val);
		++end_;
	}
	template<class T, class Alloc>
	void deque<T, Alloc>::push_front(const value_type& val){
		...
		--beg_;
		SmallSTL::construct(beg_.cur_, val);
	}
	template<class T, class Alloc>
	...
	void deque<T, Alloc>::swap(deque<T, Alloc>& x){
		SmallSTL::swap(mapSize_, x.mapSize_);
		SmallSTL::swap(map_, x.map_);
		beg_.swap(x.beg_);
		end_.swap(x.end_);
	}
